...
//	print("# Smoothing factor (Gauss): "+SF);
	...
	print("strain,medium,condition,frame#,cell#,area_fraction(patch_vs_ROI),density(analyze_particles),density(find_maxima),size,size_SD,length,length_SD,mean_patch_intensity,width,width_SD");
...
//					saveAs("PNG", PatchesDir+list[i]+"-"+j);
			...
			MEAN2_BC=MEAN2-BC;
			print(title+","+j+1+","+area_fraction+","+density+","+density2+","+size+","+size_SD+","+length+","+length_SD+","+MEAN2_BC+","+width+","+width_SD);	
		}
		close("*");
	}
}
